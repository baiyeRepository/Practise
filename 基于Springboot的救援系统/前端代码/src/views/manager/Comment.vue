<template>
  <div>
    <div class="search">
      <el-input placeholder="请输入留言人关键字查询" style="width: 200px" v-model="userName"></el-input>
  ...
  methods: {
    ...
    del(id) {   // 单个删除
      this.$confirm('您确定删除吗？', '确认删除', { type: "warning" }).then(response => {
        this.$request.delete('/comment/CompleteDelete/' + id).then(res => {
    ...
    delBatch() {   // 批量删除
      ...
      this.$confirm('您确定批量删除这些数据吗？', '确认删除', { type: "warning" }).then(response => {
        this.$request.delete('/comment/CompleteDelete/batch', { data: this.ids }).then(res => {
          if (res.code === '200') {   // 表示操作成功
            this.$message.success('操作成功')
            this.load(1)
          } else {
